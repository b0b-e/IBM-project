...
- `App/` : application source and helpers
  ...
  - `test_load_model.py` : diagnostic script that inspects model pickle headers and attempts different load strategies (`joblib` / `pickle`). Useful when loading fails.
  - `verify_model.py`, `verify_updated_model.py` : helper scripts used to (re)train or verify models in this repo (path-handling and quick verification utilities).
  - `comparison.py` : helper for model comparisons/experiments.
